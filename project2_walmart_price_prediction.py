...
Original file is located at
    https://colab.research.google.com/drive/1Vab9_bj1NGwh3SXpOXOsdbcI_D8fQFan
...
most_recent_date = proj2_data_train['Date'].max()
tmp_train = proj2_data_train[proj2_data_train['Date'] == most_recent_date].copy()
tmp_train.rename(columns={'Weekly_Sales': 'Weekly_Pred'}, inplace=True)
tmp_train = tmp_train.drop(columns=['Date', 'IsHoliday'])
tmp_train

test_pred = proj2_data_test.merge(tmp_train, on=['Dept', 'Store'], how='left')
test_pred
...
train = pd.read_csv(f'{path_to_data}/fold_10/train.csv')
test = pd.read_csv(f'{path_to_data}/fold_10/test.csv')
